...
test('supervision : pause et reprise des passages sans exécution du traitement',async({page})=>{
 await page.goto('/admin/equipes');await expect(page.getByRole('heading',{name:'Travail et commandes'})).toBeVisible();
...
test('supervision : le plan d’absence peut être enregistré sans créer de nouveaux accès',async({page})=>{
 await page.goto('/admin/autonomie');await page.getByText('Délai d’absence et consignes',{exact:true}).click();await page.getByLabel('Absence à signaler après combien de jours ?').fill('7');await page.getByLabel('Consignes en cas d’absence').fill('Continuer les traitements autorisés et conserver les décisions sensibles.');await page.getByRole('button',{name:'Enregistrer le plan'}).click();await expect(page.getByRole('status')).toContainText('plan de continuité est enregistré');
});
