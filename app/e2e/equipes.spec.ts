...
test('supervision : pause et reprise des passages sans exécution du traitement',async({page})=>{
 await page.goto('/admin/equipes');await expect(page.getByRole('heading',{name:'Vos équipes au travail'})).toBeVisible();
 ...
 await mission.getByText('Derniers passages',{exact:true}).click();await expect(mission.getByText('Aucun passage enregistré depuis la mise en place de ce suivi.')).toBeVisible();expect(await debordementHorizontal(page)).toBe(0);
});
test('supervision : le plan d’absence peut être enregistré sans créer de nouveaux accès',async({page})=>{
 await page.goto('/admin/equipes');await page.getByLabel('Absence à signaler après combien de jours ?').fill('7');await page.getByLabel('Consignes en cas d’absence').fill('Continuer les traitements autorisés et conserver les décisions sensibles.');await page.getByRole('button',{name:'Enregistrer le plan'}).click();await expect(page.getByRole('status')).toContainText('plan de continuité est enregistré');
});
test('supervision : la marque blanche donne des étapes lisibles',async({page})=>{
 await page.goto('/admin/marque-blanche');await expect(page.getByRole('heading',{name:'Les agences sous leur propre marque'})).toBeVisible();await expect(page.getByRole('link',{name:'Modifier le profil et l’apparence'}).first()).toBeVisible();expect(await debordementHorizontal(page)).toBe(0);
});
