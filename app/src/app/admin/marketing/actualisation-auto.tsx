...
export function ActualisationAuto({ actualiseLe }: { actualiseLe: string }) {
  ...
  // Le fuseau explicite évite un affichage différent au chargement du navigateur.
  return <div className="flex flex-wrap items-center gap-2 text-xs text-[var(--texte-secondaire)]"><span>Mis à jour à <time dateTime={actualiseLe}>{new Date(actualiseLe).toLocaleTimeString("fr-FR", { hour: "2-digit", minute: "2-digit", timeZone: "Europe/Paris" })}</time> (heure de Paris)</span><button type="button" onClick={actualiser} disabled={attente} className="lien-discret">{attente ? "Actualisation…" : "Actualiser"}</button></div>;
}
