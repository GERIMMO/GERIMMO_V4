...
function Etape({ faite, titre, detail }: { faite: boolean; titre: string; detail: string }) {
  return <div className="flex gap-3 rounded-xl border border-[var(--sur-marque)]/10 bg-[var(--sur-marque)]/10 p-3.5 backdrop-blur-sm">
    {faite ? <CheckCircle2 className="mt-0.5 size-5 shrink-0 text-[var(--success-soft)]" /> : <TriangleAlert className="mt-0.5 size-5 shrink-0 text-[var(--warning-soft)]" />}
    <div><p className="font-semibold text-[var(--sur-marque)]">{titre}</p><p className="mt-0.5 text-xs leading-relaxed text-[var(--sur-marque)]/70">{detail}</p></div>
  </div>;
}

function Capacite({ icone, titre, detail, statut }: { icone: React.ReactNode; titre: string; detail: string; statut: string }) {
  return <article className="group rounded-2xl border border-[var(--filet)] bg-[var(--carte)] p-5 shadow-[var(--ombre-portee)] transition hover:-translate-y-0.5 hover:shadow-[var(--ombre-flottante)]">
    <div className="flex items-start justify-between gap-3"><span className="grid size-10 place-items-center rounded-xl bg-[var(--marque-clair)] text-[var(--marque)]">{icone}</span><span className="rounded-full bg-[var(--success-soft)] px-2.5 py-1 text-[11px] font-semibold text-[var(--success-soft-foreground)]">{statut}</span></div>
...
export default async function PageAgentMarketing() {
  ...
  return <main className="mx-auto w-full max-w-7xl flex-1 space-y-7 p-4 sm:p-7">
    <section className="relative overflow-hidden rounded-[28px] bg-[var(--encre)] text-[var(--sur-marque)] shadow-[var(--ombre-flottante)]">
      ...
      <div className="absolute inset-0 bg-gradient-to-r from-[var(--encre)] via-[var(--marque-sombre)]/95 to-[var(--marque-sombre)]/55" />
      <div className="relative grid gap-7 p-6 sm:p-8 lg:grid-cols-[1.15fr_.85fr] lg:p-10">
        <div className="flex flex-col justify-between gap-8">
          <div><div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[.18em] text-[var(--marque-clair)]"><Sparkles className="size-4" /> Équipe marketing autonome</div><h1 className="mt-4 max-w-3xl font-heading text-3xl leading-tight text-[var(--sur-marque)] sm:text-4xl">Gerimmo transforme son expertise en visibilité, semaine après semaine.</h1><p className="mt-4 max-w-2xl text-sm leading-7 text-[var(--sur-marque)]/75">L’agent choisit des sujets utiles, prépare les articles, les diffuse sur Facebook et centralise les résultats. Vous gardez ici les décisions, le budget et l’historique.</p></div>
          <div className="flex flex-wrap gap-3"><Link href="/admin/publications/nouvelle" className="inline-flex items-center gap-2 rounded-xl bg-[var(--carte)] px-4 py-2.5 text-sm font-semibold text-[var(--marque-sombre)] shadow-lg"><PenLine className="size-4" /> Créer un contenu</Link>{prochaineAction && <Link href={`/admin/publications/${prochaineAction.id}`} className="inline-flex items-center gap-2 rounded-xl border border-[var(--sur-marque)]/25 bg-[var(--sur-marque)]/10 px-4 py-2.5 text-sm font-semibold text-[var(--sur-marque)] backdrop-blur">Traiter la prochaine action <ArrowRight className="size-4" /></Link>}</div>
        </div>
    ...
    <section aria-label="Situation marketing" className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
      <div className="rounded-2xl border border-[var(--marque-clair)] bg-[var(--marque-clair)] p-5"><div className="flex items-center justify-between"><span className="text-sm font-semibold text-[var(--marque-sombre)]">Publications à traiter</span><PenLine className="size-5 text-[var(--marque)]" /></div><div className="mt-2 text-3xl font-semibold text-[var(--encre)]">{aRelire.length + aDiffuser.length}</div><p className="mt-1 text-xs text-[var(--texte-2)]">{aRelire.length} à relire · {aDiffuser.length} à diffuser</p></div>
      <div className="rounded-2xl border border-[var(--success-soft)] bg-[var(--success-soft)] p-5"><div className="flex items-center justify-between"><span className="text-sm font-semibold text-[var(--success-soft-foreground)]">Facebook</span><Share2 className="size-5 text-[var(--success)]" /></div><div className="mt-2 text-3xl font-semibold text-[var(--success-soft-foreground)]">{parutionsFacebook.length}</div><p className="mt-1 text-xs text-[var(--texte-2)]">publications envoyées · {facebook.abonnes == null ? "audience en lecture" : `${nombre(facebook.abonnes)} abonnés`}</p></div>
      <div className="rounded-2xl border border-[var(--warning-soft)] bg-[var(--warning-soft)] p-5"><div className="flex items-center justify-between"><span className="text-sm font-semibold text-[var(--warning-soft-foreground)]">Campagnes à venir</span><CalendarDays className="size-5 text-[var(--warning)]" /></div><div className="mt-2 text-3xl font-semibold text-[var(--warning-soft-foreground)]">{futures.length}</div><p className="mt-1 text-xs text-[var(--texte-2)]">planning modifiable avant diffusion</p></div>
      <div className="rounded-2xl border border-[var(--destructive-soft)] bg-[var(--destructive-soft)] p-5"><div className="flex items-center justify-between"><span className="text-sm font-semibold text-[var(--destructive-soft-foreground)]">Dépenses ce mois</span><CircleDollarSign className="size-5 text-[var(--destructive)]" /></div><div className="mt-2 text-3xl font-semibold text-[var(--destructive-soft-foreground)]">{argent(depenseMois)}</div><p className="mt-1 text-xs text-[var(--texte-2)]">budget autorisé : {argent(reglages.budget_mensuel_cents)}{depenses.erreur && <span className="mt-1 block">{depenses.erreur}</span>}</p></div>
    </section>
