...
export default async function PageAgentMarketing() {
  ...
  return <main className="mx-auto w-full max-w-6xl flex-1 space-y-6 p-4 sm:p-7">
    ...
    <section className="section-ecran grid gap-5 lg:grid-cols-2"><div><h2>Historique publicitaire</h2><p className="mt-1 text-sm text-[var(--texte-secondaire)]">Anciennes campagnes Meta et résultats conservés par la plateforme.</p><div className="mt-3 divide-y divide-[var(--filet)] border border-[var(--filet)] bg-[var(--ivoire)]">{anciennesMeta.length === 0 ? <p className="p-4 text-sm text-[var(--texte-secondaire)]">Aucune ancienne publicité trouvée.</p> : anciennesMeta.map((c) => <div key={c.id} className="p-4 text-sm"><div className="flex justify-between gap-2"><b>{c.nom}</b><span className="puce puce-grise">{c.statut}</span></div><p className="mt-2 text-[var(--texte-secondaire)]">{nombre(c.portee)} personnes · {nombre(c.clics)} clics · {argent(Math.round(c.depense * 100))}</p></div>)}</div></div>
      <div><h2>Publications Facebook</h2><p className="mt-1 text-sm text-[var(--texte-secondaire)]">Articles envoyés depuis le Journal Gerimmo, ou prêts à être diffusés.</p><div className="mt-3 divide-y divide-[var(--filet)] border border-[var(--filet)] bg-[var(--ivoire)]">{aDiffuser.map((p) => <Link key={p.id} href={`/admin/publications/${p.id}`} className="block p-4 hover:bg-[var(--survol)]"><div className="flex justify-between gap-2"><b>{p.titre}</b><span className={`puce ${p.facebook_erreur ? "puce-prep" : "puce-grise"}`}>{p.facebook_erreur ? "Action requise" : "À diffuser"}</span></div><p className="mt-1 text-xs text-[var(--texte-secondaire)]">{p.facebook_erreur || "L’article est en ligne et attend sa diffusion Facebook."}</p></Link>)}{parutionsFacebook.map((p) => <Link key={p.id} href={`/admin/publications/${p.id}`} className="block p-4 hover:bg-[var(--survol)]"><div className="flex justify-between gap-2"><b>{p.titre}</b><span className="puce puce-loue">Publiée</span></div><p className="mt-1 text-xs text-[var(--texte-secondaire)]">{date(p.facebook_publie_le)} · référence {p.facebook_post_id}</p></Link>)}{aDiffuser.length === 0 && parutionsFacebook.length === 0 && <p className="p-4 text-sm text-[var(--texte-secondaire)]">Aucune publication Facebook envoyée pour le moment.</p>}</div></div>
    </section>
