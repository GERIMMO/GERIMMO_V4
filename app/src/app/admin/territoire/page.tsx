...
export default async function PageTerritoire() {
  ...
  return <main className="mx-auto w-full max-w-7xl flex-1 space-y-6 p-4 sm:p-7">
    <section className="relative overflow-hidden rounded-3xl bg-[var(--encre)] text-[var(--sur-marque)] shadow-[var(--ombre-flottante)]">
      ...
      <div className="absolute inset-0 bg-gradient-to-r from-[var(--encre)] via-[var(--marque-sombre)]/90 to-[var(--marque-sombre)]/40" />
      <div className="relative p-6 sm:p-9"><p className="flex items-center gap-2 text-xs font-semibold uppercase tracking-widest text-[var(--marque-clair)]"><MapPin className="size-4" /> Équipe développement territorial</p><h1 className="mt-3 font-heading text-3xl text-[var(--sur-marque)] sm:text-4xl">Grandir là où Gerimmo peut rendre service.</h1><p className="mt-4 max-w-3xl text-sm leading-7 text-[var(--sur-marque)]/85">Comparez le marché locatif, le réseau d’artisans et les résultats commerciaux. Gerimmo prépare une priorité et explique ce qu’il faut encore vérifier avant de recruter.</p></div>
    </section>
    ...
    <section aria-label="Présence et connaissance du marché" className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
      {[['Départements présents', echec ? '—' : empreinte.lignes.length, `${regions.length} régions couvertes`, 'bg-[var(--marque-clair)]'], ['Baux en cours', echec ? '—' : empreinte.lignes.reduce((n,l) => n+l.bauxEnCours,0), 'activité réelle des utilisateurs', 'bg-[var(--success-soft)]'], ['Marché locatif documenté', connaissance.filter(x => x.logements_loues_prive != null).length, 'départements · données INSEE', 'bg-[var(--warning-soft)]'], ['Réseau vérifié', connaissance.filter(x => x.artisans_disponibles != null).length, 'départements dont la couverture est connue', 'bg-[var(--turquoise-doux)]']].map(([titre, valeur, detail, fond]) => <div key={String(titre)} className={`rounded-2xl border border-[var(--filet)] p-5 ${fond}`}><p className="text-sm font-semibold text-[var(--bleu)]">{titre}</p><p className="mt-2 text-3xl font-semibold text-[var(--bleu)]">{valeur}</p><p className="mt-1 text-xs text-[var(--texte-secondaire)]">{detail}</p></div>)}
    </section>
