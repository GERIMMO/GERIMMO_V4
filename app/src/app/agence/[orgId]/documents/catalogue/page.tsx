...
export const metadata={title:"Catalogue de documents — Gerimmo"};
export default async function PageCatalogue({params,searchParams}:{params:Promise<{orgId:string}>;searchParams:Promise<Record<string,string|string[]|undefined>>}) {
  ...
  const recherche=valeur("q").trim(),famille=valeur("famille"),bail=valeur("bail");
  const page=Math.min(10000,Math.max(0,Number.parseInt(valeur("page"),10)||0));
  const disponibles=CATALOGUE_DOCUMENTS.filter(m=>(organisation.type==="agence"||m.famille!=="Mandats et agence") && (role!=="agent"||!["cloture_mensuelle","recap_fiscal_agence","rapport_gestion","bordereau_versement"].includes(m.code)));
  ...
  const filtres=disponibles.filter(m=>(!famille||m.famille===famille)&&(!recherche||normaliser(`${m.nom} ${m.description}`).includes(normaliser(recherche))));
  const familles=[...new Set(disponibles.map(m=>m.famille))];
  ...
  return <main className="mx-auto w-full max-w-6xl flex-1 space-y-7 p-4 sm:p-7">
    <header className="entete-page"><div><Link href={`/agence/${orgId}/documents`} className="text-sm text-muted-foreground hover:underline">Documents</Link><h1>Préparer un document</h1><p className="mt-2 max-w-2xl text-sm text-muted-foreground">Choisissez le document utile à votre situation. Gerimmo reprend les informations du dossier et conserve le PDF dans votre espace Documents.</p></div></header>
    {choisi?<><Link href={base} className="text-sm text-primary hover:underline">← Tous les modèles</Link><section className="rounded-2xl border bg-card p-5 sm:p-7"><p className="text-xs font-medium uppercase tracking-wide text-muted-foreground">{choisi.famille}</p><h2 className="mt-2 text-2xl font-semibold">{choisi.nom}</h2><p className="mb-6 mt-2 text-sm text-muted-foreground">{choisi.description}</p>
      {!Object.hasOwn(MODELES,choisi.code)?<p role="status" className="rounded-lg border p-4 text-sm">Ce modèle est en préparation. Les factures doivent provenir d’une facturation numérotée ; aucun justificatif provisoire ne sera présenté comme une facture définitive.</p>:
    ...
    </section></>:<><form className="flex flex-wrap items-end gap-3 rounded-xl border bg-card p-4"><label className="min-w-56 flex-1 text-sm font-medium">Rechercher<input name="q" defaultValue={recherche} placeholder="Bail, charges, intervention…" className="mt-2 w-full rounded-lg border bg-background px-3 py-2"/></label><label className="text-sm font-medium">Situation<select name="famille" defaultValue={famille} className="mt-2 block rounded-lg border bg-background px-3 py-2"><option value="">Toutes les situations</option>{familles.map(f=><option key={f}>{f}</option>)}</select></label><button type="submit" className={buttonVariants({variant:"secondary"})}>Rechercher</button>{(recherche||famille)&&<Link href={base} className="text-sm underline">Effacer les filtres</Link>}</form>
      <p className="text-sm text-muted-foreground">{filtres.length} modèles présentés · {filtres.filter(m=>Object.hasOwn(MODELES,m.code)).length} disponibles à la génération</p>
      {familles.filter(f=>filtres.some(m=>m.famille===f)).map(f=><section key={f}><h2 className="mb-3 text-lg font-semibold">{f}</h2><div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">{filtres.filter(m=>m.famille===f).map(m=><Link key={m.id} href={lien(m.id)} className="group flex flex-col rounded-xl border bg-card p-5 transition-colors hover:border-primary hover:bg-accent/30 focus-visible:outline-2 focus-visible:outline-primary"><h3 className="font-semibold text-primary">{m.nom}</h3><p className="mb-4 mt-2 flex-1 text-sm text-muted-foreground">{m.description}</p><span className="text-xs font-medium">{Object.hasOwn(MODELES,m.code)?"Choisir un dossier →":"En préparation"}</span></Link>)}</div></section>)}
      {!filtres.length&&<p>Aucun modèle ne correspond à cette recherche.</p>}
    </>}
  </main>;
}
