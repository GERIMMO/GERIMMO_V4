...
export default async function PageProfil(props: PageProps<"/agence/[orgId]/profil">) {
  ...
  const { data: profil, error: erreurProfil } = await supabase
    .from("organizations")
    .select(
      "name, address_line1, postal_code, city, telephone, email_contact, siret, carte_pro, garantie_financiere, iban, tva_intracom, tva_franchise, quittances_envoi_auto, appels_envoi_auto, relances_envoi_auto, relance_1_jours, relance_2_jours, code_parrainage, logo_url, couleur_primaire, couleur_secondaire, domaine_personnalise, domaine_personnalise_verifie_le, email_expediteur, email_expediteur_verifie_le, nom_portail"
    )
    .eq("id", orgId)
    .maybeSingle();
