...
export async function MesureAutonomie({orgId,detail=false,titre='Ce que Gerimmo fait pour vous'}:{orgId?:string;detail?:boolean;titre?:string}) {
 ...
 return <section className="section-ecran space-y-4">
  ...
  <div className="grid grid-cols-2 gap-3 lg:grid-cols-3">{tuiles.map(([nom,cle,aide])=><div key={nom} className="kpi bleu"><span className="libelle-champ">{nom}</span><strong className="chiffre block">{m?m[cle]:'—'}</strong><span className="mono-discret sans-majuscules mt-1 block">{aide}</span></div>)}</div>
  ...
  {detail&&m&&<div className="rounded-xl border border-border bg-white p-4"><h3 className="mb-3 font-semibold">Par organisation</h3>
