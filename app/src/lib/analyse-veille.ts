...
// La citation doit venir de la source, mais pas au caractère près : le modèle
// remplace volontiers l'apostrophe typographique, les guillemets et les blancs
...
export function validerEtudes(reponse:unknown,sources:SourceEtude[]):AnalyseVeille[]{
 ...
 for(const a of analyses){
  ...
  if(a.application!==null&&!dateValide(a.application))throw new Error('Date d’application invalide.');
  vus.add(a.id);
 }
 return analyses;
}
