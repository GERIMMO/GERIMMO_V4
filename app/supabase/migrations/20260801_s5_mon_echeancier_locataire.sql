-- Sprint 5C : le locataire consulte son échéancier + ses quittances.
create function public.mon_echeancier_locataire(p_org uuid)
returns table (periode date, montant_du numeric, montant_couvert numeric, statut text, a_quittance boolean)
language sql
stable
security definer
set search_path = ''
as $$
  select e.periode, e.montant_du, e.montant_couvert, e.statut,
    exists (select 1 from public.quittances q where q.appel_id = e.appel_id) as a_quittance
  from public.baux b
