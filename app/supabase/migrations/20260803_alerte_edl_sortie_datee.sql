-- L'alerte « état des lieux de sortie » n'avait pas d'échéance.
--
-- La date était pourtant connue — elle voyage dans le détail de l'alerte sous
-- `date_effet` — mais la colonne `echeance` restait nulle. Conséquence sur le
-- tableau de bord : l'alerte tombait dans « à venir » avec la mention « sans
-- échéance », sans jamais remonter à l'approche de la sortie. L'agent pouvait
-- rater l'état des lieux, et donc la restitution du dépôt dans les délais.
--
-- L'état des lieux de sortie se fait le jour du départ : l'échéance est la date
-- d'effet du congé.
...
-- Rattrapage des alertes déjà créées : la date dort dans leur détail.
update public.alerts
   set echeance = (details->>'date_effet')::date
 where type = 'edl_sortie'
   and echeance is null
   and details ? 'date_effet';
