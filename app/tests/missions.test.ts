...
describe('Commande des missions',()=>{
 ...
 it('classe un échec partiel même avec une réponse HTTP réussie',()=>{expect(bilanMission({envoyees:3,echecs:1},200)).toEqual({ok:false,compte:3});expect(bilanMission({erreur:'secret'},200).ok).toBe(false);expect(bilanMission({},503).ok).toBe(false);expect(bilanMission({erreurFacebook:'refus'},200).ok).toBe(false);expect(bilanMission({orchestration_erreur:true},200).ok).toBe(false);});
