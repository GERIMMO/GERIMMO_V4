...
## [2026-09-25] implementation | Variables Vercel — contrôle bloqué dans la session, outil de vérification et de pose

**Demande** : « vérifier les variables Vercel et poser ce qui manque ».

**Bloqué dans cette session** : le connecteur Vercel refuse le jeton de la session (« sign in again ») ; la politique réseau de l'environnement bloque aussi `api.vercel.com` et `gerimmo.app` (403 au CONNECT), donc ni l'API avec un jeton, ni la lecture de l'écran Santé en production ne sont possibles d'ici. Aucune variable n'a pu être lue ni posée. Pour débloquer : reconnecter le connecteur Vercel sur claude.ai (Paramètres > Connecteurs), puis ouvrir une **nouvelle** session — les connecteurs sont lus au démarrage.

**Fait, pour que la pose tienne en une commande** : `app/scripts/vercel/variables.mjs` (catalogue des 27 variables que la production lit, en trois groupes : socle, prestataires, facultatives ; comparaison avec ce que porte le projet, noms et cibles seulement ; contrôle de forme avant toute pose — clé secrète sous `NEXT_PUBLIC_`, clé publiable sous `SUPABASE_SERVICE_ROLE_KEY`, adresse locale, secret trop court — ; génération de `CRON_SECRET`) et `verifier.mjs` (API Vercel avec `VERCEL_TOKEN`, `VERCEL_PROJECT_ID`, `VERCEL_TEAM_ID`, les noms de la publication contrôlée ; rapport sans valeur ; `--poser <fichier .env>` crée les manquantes en production sans rien écraser). Neuf tests `node:test` ajoutés à la CI, un test vitest qui garde le catalogue aligné sur `etatConfiguration` de l'écran Santé. Procédure dans `app/docs/variables-vercel.md`.

**Valeurs disponibles sans le porteur** : adresse Supabase `https://rddlxunppddzpsaatdaz.supabase.co` et clé publiable (Dashboard > API Keys) — certainement déjà posées puisque le site tourne ; adresse publique `https://www.gerimmo.app` (celle des scripts de publication ; à confirmer que c'est le domaine canonique sur Vercel). **Au porteur** : `SUPABASE_SERVICE_ROLE_KEY` (Supabase > API Keys), les quatre Stripe (même mode), Resend, Yousign, et les facultatives (OpenAI, Meta). `CRON_SECRET` se génère seul.

## [2026-09-25] implementation | Variables Vercel — seconde tentative : quatre voies fermées, ce que la production laisse voir

Le porteur a répété la demande après avoir reconnecté le connecteur Vercel. Projet identifié : `gerimmo-v4` (`prj_YdwqVmhZU5ZDABPZom8OkWcXa1FV`, équipe `gerimmo`, `team_dXzIokNudrLR87DxLPeIYaGu`), domaines `www.gerimmo.app` et `gerimmo.app`, production sur `c1db5f4`. **Quatre voies fermées** : le connecteur n'a aucun outil qui lise ou pose une variable d'environnement (vérifié sur la liste complète) ; le réseau du conteneur refuse toujours `api.vercel.com` ; aucun secret dans l'environnement ; le secret GitHub `VERCEL_TOKEN` est vide (constaté par le nouveau workflow, run 36176129465). Aucune variable n'a donc été posée.

**Vérifié malgré tout, par les traces.** Production : `/api/sante` répond `ok`/`base` ; neuf tâches consignent leurs passes chaque jour, donc `CRON_SECRET` et `SUPABASE_SERVICE_ROLE_KEY` sont posées ; la passe abonnements n'est pas `non_configuree`, donc `STRIPE_SECRET_KEY` et `STRIPE_WEBHOOK_SECRET` sont posées ; Resend répond (clé posée) mais **refuse les quittances depuis le 21/09** (destinataire en `example.com`, puis refus générique du 23 au 25) ; OpenAI répond (clé posée) mais **« plus de crédit » le 25/09** (veille réglementaire bloquée). Preview : `/api/sante` de la branche répond 503 « SUPABASE_SERVICE_ROLE_KEY absente » — **manque vérifié sur la cible preview**. **Anomalie** : `tache_signatures` n'a aucune passe consignée sur sept jours alors que la route consigne même en sandbox depuis le 25/09, et aucun log d'exécution n'apparaît ; à élucider. Invisible d'ici : `NEXT_PUBLIC_SITE_URL` (aucune page publique ne l'affiche), Yousign, Meta, GitHub.

**Ajouté** : `.github/workflows/variables-vercel.yml` — lance `scripts/vercel/verifier.mjs` avec le secret de la publication contrôlée, rapport (noms et cibles) dans le résumé du job ; manuel depuis `main`, automatique sur un push de ses propres fichiers ; nomme le secret absent. Ce qui débloque tout : **poser `VERCEL_TOKEN` en secret GitHub** (Settings > Secrets and variables > Actions) et relancer le run, ou lancer le script depuis un poste. La pose des valeurs reste au porteur (`--poser .env.local`).

## [2026-09-25] implementation | Variables Vercel — suite : l'anomalie des signatures expliquée, les destinataires Resend partis avec la purge

Sur « je te laisse faire tout ça ». **Signatures** : `agent_passages` montre bien une passe le 25/09 à 03:15 UTC, 33 ms, « à reprendre », sans ligne dans `tech_log` : c'est l'ancien code, déployé jusqu'à 16:23 UTC, qui répondait 503 avant tout journal quand Yousign n'est pas en production. Le correctif du 25/09 consigne désormais `non_configuree` ; la passe du 26/09 à 03:00 UTC le montrera dans Santé. Conséquence utile : **Yousign n'est pas configuré en production** (clé absente ou sandbox). **Resend** : la base ne contient plus qu'une seule adresse e-mail, celle du compte du porteur ; les refus sur `example.com` venaient des données de démonstration purgées le 25/09, la passe quittances du 26/09 n'aura rien à envoyer. Reste le domaine d'expédition à vérifier chez Resend avant les premiers vrais locataires. **Secret GitHub `VERCEL_TOKEN`** : toujours absent au second run (36176380727). Rendez-vous de contrôle posé pour le 26/09 au matin : relire la passe signatures, relancer le rapport si le secret est arrivé.

Toujours au porteur, parce que cela passe par ses comptes : le jeton Vercel en secret GitHub, les crédits OpenAI, le domaine Resend, les valeurs Stripe réel et Yousign, et `SUPABASE_SERVICE_ROLE_KEY` sur la cible preview.
