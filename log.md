...
## [2026-09-25] mise en production | Fin de soirée : alerte de sauvegarde dans Gerimmo, veille au vert, restes de démonstration effacés

PR #112 fusionnée et déployée : chaque passage de sauvegarde s'inscrit dans `tech_log` (`tache_sauvegarde`), ligne « Sauvegarde hebdomadaire » dans Santé, bloquante en cas d'échec ou de retard de plus de huit jours ; second passage réussi et consigné. Veille réglementaire : après journalisation des motifs (date d'application avec heure, délai de 25 s), corrections déployées, passage au vert (trois actualités étudiées). Le porteur a supprimé les 61 fichiers de démonstration du Storage : le bucket `documents` est vide, la production ne contient plus aucune donnée de démonstration. Restent : Yousign (deux clés), les huit faits légaux, la règle de cycle de vie à 90 jours chez Scaleway, la recharge automatique OpenAI.

## [2026-09-26] visuel | Nuit du 25 au 26/09 : retour à la respiration du 24/09, menus fixes

Le porteur, devant la version en ligne : « le visuel est trop strict, pas assez respirant, le menu est déroulant, je veux le menu fixe », « je veux vraiment l'ancien visuel », puis « compare l'ancien et le nouveau et corrige pour faire quelque chose de cohérent » et carte blanche pour la nuit. Méthode : l'état approuvé du 24/09 (`fbde628`) relancé sur un second banc, 136 écrans capturés des deux côtés (bureau 1280 px et téléphone 390 px), quatre agents par périmètre (console, agence, propriétaire + locataire, artisan + public) avec la même consigne : référence = 24/09, contenu du 25/09 gardé, pas de style ad hoc, propositions CSS déposées à part puis intégrées au bloc `RESPIRATION` de `globals.css`. Livré sur la branche en cinq commits (`95d39d3`, `34c620b`, `945b117`, `499c382`, `32853c0`) : menus fixes (console et agence), jetons de respiration, titre de carte au pas de 16 px, cartes de grille alignées, formulaires desserrés et champs fichier en français, console sur `.loc-carte`/`.colonne-liste`/`.rang`, Territoire sans débordement au téléphone, cartes artisan et tableau des prestataires. Non tranché, pour le porteur au réveil : rubriques repliables du dossier de bail et groupes « À débloquer / À venir » du tableau de bord (demande antérieure du porteur, figée dans un test), densité du menu de la console (17 entrées et 5 titres contre 11 avant), boutons « Mon entreprise » de l'espace artisan face au formulaire du 24/09. Détail dans [[Design system Gerimmo]] § 5.

## [2026-09-26] decision | Console : plan à sept entrées dessiné par le porteur

Le porteur a proposé le rangement de la console : « Vue d'ensemble (avec aujourd'hui) — Utilisateurs (artisans à valider, demandes commerciales, agences, bailleurs, artisans, personnalisation des agences) — Veille (réglementaire, santé et connexions, relais en mon absence) — Marketing (agent marketing, articles du site) — Développement (développement du site : dossiers et évolutions, retours des utilisateurs ; développement territorial) — Historique et conservation — Paramètres », avec « Travail et commandes » rangé dans Veille, et carte blanche pour agencer les pages. Livré sur la PR #115 : la colonne passe de 23 lignes à 7 entrées ; chaque rubrique montre ses pages en onglets sous le bandeau (un seul tableau, `lib/rubriques-supervision.ts`, lu par le menu et les onglets) ; les adresses des pages ne changent pas. « Travail et commandes » devient « Travail des équipes », avec une phrase qui dit ce que c'est (ce que Gerimmo fait seul chaque jour, et les boutons pour relancer ou suspendre). « Relais en mon absence » quitte la page des dossiers pour sa propre page dans Veille. Nouvelle page Paramètres : identité de l'éditeur (les huit faits exigés, fournis ou non), sécurité du compte, ouverture d'une organisation. Voir [[Design system Gerimmo]] § 5.
