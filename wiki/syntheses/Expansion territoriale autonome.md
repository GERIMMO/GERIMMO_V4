...
| Parrainage | **mécanique construite le 19/09** ([[Parrainage]]) : un code par organisation engendré à la création, saisi à l'auto-inscription (ou porté par le lien `/inscription?parrain=…`) et à l'ouverture d'une agence par la supervision, un parrain au plus par filleul, lecture par les deux parties, écriture par une seule fonction ; carte « Parrainage » sur le profil. **Migration `20260919120000` à appliquer** par le chantier « Migrations Supabase » avant la mise en ligne du code. L'**avantage** reste à trancher. |
